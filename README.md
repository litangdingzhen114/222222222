...
- 阿里云生产 API 预留 `https://api.hailin.store`，等待 ICP/HTTPS 完整放通后启用
- 小程序合法 request/upload/download 域名优先配置 `https://www.hailin.store`，`https://api.hailin.store` 完成放通后保留为阿里云直连备用
